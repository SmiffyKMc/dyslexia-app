...
void main() {
  group('PromptLoader Tests', () {
    ...
    test('should load all text simplifier templates', () async {
      // Test base template
      final baseTemplate = await PromptLoader.load('text_simplifier', 'base.tmpl');
      ...
      final explainChanges = await PromptLoader.load('text_simplifier', 'explain_changes.tmpl');
      expect(explainChanges, contains('EXPLAIN CHANGES'));
      
      final defineTerms = await PromptLoader.load('text_simplifier', 'define_terms.tmpl');
      expect(defineTerms, contains('DEFINE KEY TERMS'));
      
      final addVisuals = await PromptLoader.load('text_simplifier', 'add_visuals.tmpl');
      expect(addVisuals, contains('VISUAL SUGGESTIONS'));
      
      final regeneration = await PromptLoader.load('text_simplifier', 'regeneration.tmpl');
      expect(regeneration, contains('REGENERATION REQUEST'));
    });
    
    test('should load story generation templates', () async {
      final storyWithQuestions = await PromptLoader.load('story_generation', 'story_with_questions.tmpl');
      expect(storyWithQuestions, contains('educational stories for dyslexic learners'));
      ...
      final storySimple = await PromptLoader.load('story_generation', 'story_simple.tmpl');
    ...
    test('should load sentence fixer templates', () async {
      final batchGeneration = await PromptLoader.load('sentence_fixer', 'batch_generation.tmpl');
      expect(batchGeneration, contains('Generate exactly {{count}}'));
      
      final singleSentence = await PromptLoader.load('sentence_fixer', 'single_sentence.tmpl');
      expect(singleSentence, contains('Create {{count}} beginner practice sentences'));
      
      final simpleSentence = await PromptLoader.load('sentence_fixer', 'simple_sentence.tmpl');
      expect(simpleSentence, contains('Create 1 sentence with exactly 1 SPELLING mistake'));
    });
    
    test('should load profile analysis templates', () async {
      final fullUpdate = await PromptLoader.load('profile_analysis', 'full_update.tmpl');
      ...
      final minimalUpdate = await PromptLoader.load('profile_analysis', 'minimal_update.tmpl');
      expect(minimalUpdate, contains('Update profile minimally'));
      expect(minimalUpdate, contains('{{current_profile}}'));
    });
    
    test('should load word analysis templates', () async {
      final definition = await PromptLoader.load('word_analysis', 'definition.tmpl');
    ...
    test('should load shared templates', () async {
      final legacySimplification = await PromptLoader.load('shared', 'legacy_sentence_simplification.tmpl');
